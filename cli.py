...
while not gameover :
    try :
        showc = (time.strftime("%Y-%m-%d %a %H:%M:%S")+" Time that game has run: %f Vertical speed: %f Horizontal speed: %f Range of rolls: %d Score: %d") % (time.time()-start_time, eval(str(speed_v)), eval(str(speed_h)), eval(str(roll_range)), score)
        ...
        is_pausing = 1
        print("PAUSED, press enter to continue, or press Ctrl-C again to exit")
        try :
            while is_pausing :
                pass
        except KeyboardInterrupt :
